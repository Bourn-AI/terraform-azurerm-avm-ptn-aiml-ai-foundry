...
- `map key` - The key for the map entry. This key should match the dependent resources keys when creating connections.
  ...
  - `additional_connections` - (Optional) Map of additional project connections to create using the AI Foundry connections API (e.g., API Key, Custom Keys, SharePoint). Each map value supports:
    ...
    - `credentials` - (Optional) Key/value secret material (marked sensitive upstream). When `additional_connections_key_vault` (project-level) or `key_vault_secret` (per-connection) is provided, any value prefixed with `secret:` is interpreted as a Key Vault secret name and resolved from that vault; other values are sent as-is. For `auth_type = "CustomKeys"` these entries are placed under `credentials.keys` to match the schema.
    - `key_vault_secret` - (Optional) Source credentials from Key Vault instead of inline secrets. Provide vault name, resource group, and secret name; `credential_key` sets the key name in the credentials object (default "key"). For `CustomKeys`, the value is placed at `credentials.keys[credential_key]`, and any `credentials` map values prefixed with `secret:` are looked up in the same vault.
